...
exports.signup = async (req, res)=>{
    ...
    try{
       ...
       await user.save();
         ...
         const message = `${process.env.BASE_URL}/user/verify/${user.id}/${token.id}`;
...
exports.verify=async (req, res)=>{
    try {
        ...
        console.log('mehreen is a good programmer')
        ...
        if (!token) return res.status(400).send("previously verified");
