...
export const VendorSignup = (props) => {
  ...
  const onFinish = async (values) => {
    ...
    await axios.post('/api/users/vendor/signup', data).then(res => {
        setLoading(false);
        if (res.status === 200) {
    ...
    loading
      ...
      <UserLayout navbar>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '100px' }}>
          <div>
            <h2 className='text-center ml-5 my-4'>Signup as Vendor</h2>
            <Form
              {...formItemLayout}
              form={form}
              name="register"
            ...
            <div className='mt-4 text-center'>
              <p>
                Signup as Retailer? <Link to='/retailer/signup' style={{ color: '#ff5a5a', fontWeight: '700' }}>Register</Link>
              </p>
              <p>
